...
public class GasTurbineGeneratorBlockEntity extends BaseIOBlockEntity implements GeoBlockEntity {
    ...
    public GasTurbineGeneratorBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.GAS_TURBINE_GENERATOR.get(), pos, state, 400);
    }
    ...
    protected ContainerData createPropertyDelegate() {
        return new ContainerData() {
            @Override public int get(int index) {
                return switch (index) {
                    case 0 -> GasTurbineGeneratorBlockEntity.this.progress;
                    case 1 -> GasTurbineGeneratorBlockEntity.this.maxProgress;
                    case 2 -> GasTurbineGeneratorBlockEntity.this.enable ? 1 : 0;
                    default -> 0;
                };
            }
            @Override public void set(int index, int value) {
                switch (index) {
                    case 0 -> GasTurbineGeneratorBlockEntity.this.progress = value;
                    case 1 -> GasTurbineGeneratorBlockEntity.this.maxProgress = value;
                    case 2 -> GasTurbineGeneratorBlockEntity.this.enable = value == 1;
                }
            }
    ...
    private record OutputHandler(ItemStackHandler parent) implements IItemHandler {
        ...
        @Override public @NotNull ItemStack getStackInSlot(int slot) {
            return parent.getStackInSlot(GasTurbineGeneratorBlockEntity.OUTPUT_SLOT);
        }
        @Override public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
            return stack;
        }
        @Override public @NotNull ItemStack extractItem(int slot, int amount, boolean simulate) {
            return parent.extractItem(GasTurbineGeneratorBlockEntity.OUTPUT_SLOT, amount, simulate);
        }
        @Override public int getSlotLimit(int slot) { return 64; }

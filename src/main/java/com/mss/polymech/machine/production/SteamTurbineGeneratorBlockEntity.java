...
public class SteamTurbineGeneratorBlockEntity extends BaseIOBlockEntity implements GeoBlockEntity {
    ...
    public SteamTurbineGeneratorBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.STEAM_TURBINE_GENERATOR.get(), pos, state, 400);
    }
    ...
    protected ContainerData createPropertyDelegate() {
        return new ContainerData() {
            @Override public int get(int index) {
                return switch (index) {
                    case 0 -> SteamTurbineGeneratorBlockEntity.this.progress;
                    case 1 -> SteamTurbineGeneratorBlockEntity.this.maxProgress;
                    case 2 -> SteamTurbineGeneratorBlockEntity.this.enable ? 1 : 0;
                    default -> 0;
                };
            }
            @Override public void set(int index, int value) {
                switch (index) {
                    case 0 -> SteamTurbineGeneratorBlockEntity.this.progress = value;
                    case 1 -> SteamTurbineGeneratorBlockEntity.this.maxProgress = value;
                    case 2 -> SteamTurbineGeneratorBlockEntity.this.enable = value == 1;
                }
            }
    ...
    private record OutputHandler(ItemStackHandler parent) implements IItemHandler {
        ...
        @Override public @NotNull ItemStack getStackInSlot(int slot) {
            return parent.getStackInSlot(SteamTurbineGeneratorBlockEntity.OUTPUT_SLOT);
        }
        @Override public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
            return stack;
        }
        @Override public @NotNull ItemStack extractItem(int slot, int amount, boolean simulate) {
            return parent.extractItem(SteamTurbineGeneratorBlockEntity.OUTPUT_SLOT, amount, simulate);
        }
        @Override public int getSlotLimit(int slot) { return 64; }

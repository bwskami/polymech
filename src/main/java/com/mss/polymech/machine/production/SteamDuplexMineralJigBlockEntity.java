...
public class SteamDuplexMineralJigBlockEntity extends BaseIOBlockEntity implements GeoBlockEntity {
    ...
    public SteamDuplexMineralJigBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.STEAM_DUPLEX_MINERAL_JIG.get(), pos, state, 300);
    }
    ...
    protected ContainerData createPropertyDelegate() {
        return new ContainerData() {
            @Override public int get(int index) {
                return switch (index) {
                    case 0 -> SteamDuplexMineralJigBlockEntity.this.progress;
                    case 1 -> SteamDuplexMineralJigBlockEntity.this.maxProgress;
                    case 2 -> SteamDuplexMineralJigBlockEntity.this.enable ? 1 : 0;
                    default -> 0;
                };
            }
            @Override public void set(int index, int value) {
                switch (index) {
                    case 0 -> SteamDuplexMineralJigBlockEntity.this.progress = value;
                    case 1 -> SteamDuplexMineralJigBlockEntity.this.maxProgress = value;
                    case 2 -> SteamDuplexMineralJigBlockEntity.this.enable = value == 1;
                }
            }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.poly_mech.steam_duplex_mineral_jig");
    }
    ...
    private record OutputHandler(ItemStackHandler parent) implements IItemHandler {
        ...
        @Override public @NotNull ItemStack getStackInSlot(int slot) {
            return parent.getStackInSlot(SteamDuplexMineralJigBlockEntity.OUTPUT_SLOT);
        }
        @Override public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
            return stack;
        }
        @Override public @NotNull ItemStack extractItem(int slot, int amount, boolean simulate) {
            return parent.extractItem(SteamDuplexMineralJigBlockEntity.OUTPUT_SLOT, amount, simulate);
        }
        @Override public int getSlotLimit(int slot) { return 64; }

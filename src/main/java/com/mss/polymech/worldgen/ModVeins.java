...
public final class ModVeins {
    ...
    public record VeinDefinition(
        ...
        public VeinDefinition(
                ...
                Set<String> allowedRocks) {
            this(id, rarity, minY, maxY, size, size, density,
                    primary, secondary, between, sporadic, allowedRocks);
        }

        /** 基础区间 × 矿种倍率：用 BASE_SIZE_MIN/MAX 推算 min/max */
        public VeinDefinition(
                String id, int rarity, int minY, int maxY,
            ...
            this(id, rarity, minY, maxY,
                    ...
                    density, primary, secondary, between, sporadic, allowedRocks);
        }

        /** 手动自定义最大最小值：就是带 sizeMin/sizeMax 的规范构造器 */
        public static VeinDefinition ranged(
                String id, int rarity, int minY, int maxY,
                int sizeMin, int sizeMax, float density,
                String primary, String secondary,
                @Nullable String between, @Nullable String sporadic,
                Set<String> allowedRocks) {
            return new VeinDefinition(id, rarity, minY, maxY, sizeMin, sizeMax, density,
                    primary, secondary, between, sporadic, allowedRocks);
        }

        /** 是否在某个确定值范围内（供 datagen 生成 size_min/size_max） */
        public int deterministicSize(long hash) {
            int span = Math.max(0, sizeMax - sizeMin);
            int value = sizeMin + (int) ((hash & 0x7fffffff) % (span + 1));
